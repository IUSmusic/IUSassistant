(function () {
  ...
  function nearest(target, pool, windowSize) {
    ...
    const limit = Math.max(1, Math.min(windowSize, sorted.length));
  ...
  function progressionToChordPools(root, scaleName, progression) {
    ...
    const scaleBase = midiPool(root, isMinorFamily ? 'minor' : 'major', [4, 5]);
  ...
  function estimateAdaptiveStep(seed) {
    ...
    for (let i = 1; i < seed.length; i += 1) {
      const gap = Math.max(0.25, seed[i].time - seed[i - 1].time);
      gaps.push(gap);
    }
  ...
  function pushSeed(noteName, duration = 0.45, velocity = 0.8) {
    const prev = state.capturedNotes[state.capturedNotes.length - 1];
    const timeline = prev ? prev.time + prev.duration + 0.05 : 0;
  ...
  function generateTheory() {
    ...
    let time = seed[seed.length - 1] ? seed[seed.length - 1].time + seed[seed.length - 1].duration + 0.05 : 0.5;
    const out = [];
    ...
    for (let i = 0; i < noteCount; i += 1) {
      ...
      const duration = Math.max(0.24, Math.min(1.2, stepBeats * 0.28));
      ...
      lastMidi = midi;
      time += duration + Math.max(0.06, stepBeats * 0.08);
    }
    return out;
  }
  ...
  async function generateWithMagenta(modelKey) {
    ...
    const starts = sequence.notes.map((n) => n.startTime || 0);
    const firstStart = Math.min.apply(null, starts);

    return sequence.notes
      .map((n) => {
        const rawStart = (n.startTime || 0) - firstStart;
        const rawEnd = (n.endTime || ((n.startTime || 0) + 0.4)) - firstStart;
        const midi = snapToScale(n.pitch || 60, root, scaleName);
        return {
          note: midiToName(midi),
          midi,
          time: baseTime + rawStart,
          duration: Math.max(0.24, Math.min(1.2, rawEnd - rawStart)),
  ...
  function playSequence(sequence) {
    ...
    ensureAudio().then(() => {
      ...
      sequence.forEach((note) => {
        ...
        setTimeout(() => flashKey(note.note), Math.max(0, (note.time - anchor) * 1000));
      });
    });
  }
  ...
  function register() {
    ...
    [els.rootSelect, els.scaleSelect, els.engineSelect, els.playbackSelect].forEach((el) => el.addEventListener('change', updateUI));
