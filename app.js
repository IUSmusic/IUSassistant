(function () {
  ...
  function niceNote(name) { return name.replace('#', '♯'); }
  ...
  function progressionSymbols() {
    return els.progressionSelect.value.split('-').map((s) => s.trim());
  }
  function progressionChordLabel(barIndex) {
    const symbol = progressionSymbols()[barIndex % progressionSymbols().length] || 'I';
    return degreeLabel(els.rootSelect.value, els.scaleSelect.value, symbol);
  }
  function progressionToChordPool(symbol) {
    const root = els.rootSelect.value;
    const scaleName = ['minor', 'dorian', 'blues'].includes(els.scaleSelect.value) ? 'minor' : 'major';
    const pcs = getScalePitchClasses(root, scaleName);
    const degreeIndex = romanToDegree(symbol);
    const triad = [0, 2, 4].map((jump) => pcs[(degreeIndex + jump) % pcs.length]);
    return midiPool(root, scaleName, [3,4,5,6]).filter((m) => triad.includes(m % 12));
  }
  ...
  function summarizeBar(notesInBar) {
    if (!notesInBar.length) return { chord: '—', notes: 'Rest', glyphs: '𝄽' };
    const notes = [...new Set(notesInBar.slice(0, 4).map((n) => niceNote(n.note.replace(/\d+/g, ''))))];
    const glyphs = notesInBar.slice(0, 4).map((n) => noteGlyph(n.duration)).join('');
    return { chord: '', notes: notes.join(' · '), glyphs };
  }
  function attachBarMeta(clip) {
    const meta = [];
    for (let bar = 0; bar < clip.bars; bar += 1) {
      ...
      const summary = summarizeBar(notes);
      summary.chord = progressionChordLabel(bar);
      meta.push(summary);
    }
    clip.barMeta = meta;
    return clip;
  ...
  function generateTheoryForBars(bars, sectionOffset = 0, registerShift = 0) {
    const root = els.rootSelect.value;
    ...
    const scalePool = midiPool(root, scaleName, [4 + registerShift, 5 + registerShift].filter((o) => o >= 2 && o <= 7));
    ...
    for (let i = 0; i < noteCount; i += 1) {
      ...
      const symbol = progressionSymbols()[absoluteBar % progressionSymbols().length] || 'I';
      const chordPool = progressionToChordPool(symbol);
      const styled = lastMidi + styleShift(style, i + sectionOffset) * (style === 'ambient' ? 1 : 2);
  ...
  function buildClip(name, engine, bars, notes, kind) {
    return attachBarMeta({
  ...
  async function generateWithMagenta(modelKey, bars) {
    ...
    const normalized = notes.slice(0, Math.max(16, bars * 8)).map((n) => ({ ...n, time: n.time - first, note: midiToName(n.midi) }));
    return normalized;
  }
  async function generateClip() {
    try {
      ...
      const notes = engine === 'theory' ? generateTheoryForBars(bars) : await generateWithMagenta(engine, bars);
      const clip = buildClip(`${engine === 'theory' ? 'Theory' : engine === 'musicvae-2bar' ? 'VAE 2' : 'VAE 4'} · ${state.resultClips.length + 1}`, engine, bars, notes, 'clip');
      ...
      console.error(err);
      const bars = Number(els.barsSelect.value);
      const clip = buildClip(`Theory · ${state.resultClips.length + 1}`, 'theory', bars, generateTheoryForBars(bars), 'clip');
      state.resultClips.unshift(clip);
      els.modeReadout.textContent = 'Theory';
      setStatus('Theory fallback');
      updateUI();
    }
  }
  function placeClip(clipId, trackIndex, startBar) {
    ...
    for (let i = 0; i < clip.bars; i += 1) {
      state.arrangement[trackIndex][startBar + i] = { clipId, startBar, offsetBar: i };
    }
    renderArrangement();
    return true;
  }
  function firstOpenSlot(barsNeeded) {
    for (let t = 0; t < TRACKS; t += 1) {
      for (let b = 0; b <= state.arrangementBars - barsNeeded; b += 1) {
        let free = true;
        for (let i = 0; i < barsNeeded; i += 1) if (state.arrangement[t][b + i]) free = false;
        if (free) return { t, b };
      }
    }
    return { t: 0, b: 0 };
  ...
  async function generateFullTrack() {
    await ensureAudio();
    const bars = Number(els.trackBarsSelect.value);
    if (bars !== state.arrangementBars) resizeArrangement(bars);
    const style = els.styleSelect.value;
    const sections = [];
    ...
    sectionPlan.forEach((len, index) => {
      ...
      const notes = generateTheoryForBars(len, cursor, shift);
      const moved = notes.map((n) => ({ ...n, time: n.time + (cursor * BAR_BEATS) }));
      sections.push(...moved);
      cursor += len;
    });
    if (style === 'ambient') sections.forEach((n, i) => { if (i % 5 === 0) n.duration = Math.min(2.5, n.duration * 1.5); });
    const clip = buildClip(`Full Track · ${bars} bars`, 'theory', bars, sections, 'track');
    state.resultClips.unshift(clip);
    placeClip(clip.id, 0, 0);
    els.modeReadout.textContent = 'Full Track';
    setStatus('Full track ready');
    updateUI();
  }
  ...
  async function playArrangement() {
    ...
    state.arrangement.forEach((track) => {
      track.forEach((slot, barIndex) => {
        ...
        seenStarts.add(key);
        const clip = state.resultClips.find((c) => c.id === slot.clipId);
        if (!clip) return;
        scheduleSequence(clip.notes, barIndex * BAR_BEATS);
      });
    });
  ...
  function removeClipFromArrangement(trackIndex, barIndex) {
    ...
    for (let i = 0; i < state.arrangementBars; i += 1) {
      const s = state.arrangement[trackIndex][i];
      if (s && s.clipId === slot.clipId && s.startBar === slot.startBar) state.arrangement[trackIndex][i] = null;
    }
    renderArrangement();
  }
  ...
  function renderSeed() {
    ...
    state.seedNotes.forEach((note, index) => {
      ...
      row.innerHTML = `<strong>${index + 1}</strong><div>${niceNote(note.note)}<br><span>${note.time.toFixed(2)} beat · ${note.duration.toFixed(2)} beat</span></div><div>${Math.round(note.velocity * 100)}%</div>`;
  ...
  function renderResults() {
    ...
    if (!state.resultClips.length) {
      els.resultsList.innerHTML = '<div class="empty-state">Generate a clip or full track to create playable results.</div>';
      return;
    }
    state.resultClips.forEach((clip) => {
      ...
      const preview = clip.notes.slice(0, 20).map((note) => `<div class="clip-bar" style="height:${10 + ((note.midi - 48) % 24) * 1.2}px"></div>`).join('');
      const metaStrip = clip.barMeta.slice(0, Math.min(clip.barMeta.length, 4)).map((m) => `<span>${m.chord} · ${m.notes || 'Rest'} · ${m.glyphs}</span>`).join('');
      card.innerHTML = `
        <div class="clip-top">
          <div>
            <div class="clip-name">${clip.name}</div>
            <div class="clip-meta">${clip.notes.length} notes · ${clip.bars} bars · ${clip.kind === 'track' ? 'full track' : 'clip'}</div>
          </div>
      ...
      card.querySelector('.add-clip').addEventListener('click', () => {
        const slot = firstOpenSlot(clip.bars);
        placeClip(clip.id, slot.t, slot.b);
      });
  ...
  function renderArrangement() {
    ...
    els.arrangementMeta.textContent = `Bottom timeline · ${TRACKS} tracks · ${state.arrangementBars} bars`;
    ...
    for (let trackIndex = 0; trackIndex < TRACKS; trackIndex += 1) {
      ...
      for (let barIndex = 0; barIndex < state.arrangementBars; barIndex += 1) {
        ...
        cell.addEventListener('drop', (event) => {
          ...
          if (clipId) placeClip(clipId, trackIndex, barIndex);
        });
        const slot = state.arrangement[trackIndex][barIndex];
        if (slot) {
          const clip = state.resultClips.find((c) => c.id === slot.clipId);
  ...
  function flattenArrangementSequence() {
    const merged = [];
    state.arrangement.forEach((track) => {
      track.forEach((slot, barIndex) => {
        if (!slot || slot.offsetBar !== 0) return;
        const clip = state.resultClips.find((c) => c.id === slot.clipId);
        if (!clip) return;
        clip.notes.forEach((n) => merged.push({ ...n, time: n.time + (barIndex * BAR_BEATS) }));
      });
    });
    return merged;
  }
  ...
  function exportMidi() {
    const sequence = flattenArrangementSequence().length ? flattenArrangementSequence() : state.seedNotes.concat(...state.resultClips.map((clip) => clip.notes));
  ...
  function register() {
    ...
    els.generateTrackBtn.addEventListener('click', generateFullTrack);
    ...
    els.trackBarsSelect.addEventListener('change', () => { resizeArrangement(Number(els.trackBarsSelect.value)); updateUI(); });
  ...
  function init() {
    createKeyboard();
    applyTheme();
    resizeArrangement(Number(els.trackBarsSelect.value));
    register();
    updateUI();
  }
  init();
